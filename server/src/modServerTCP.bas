...
Sub SendWhosOnline(ByVal index As Long)
    ...
    Call PlayerMsg(index, s, BrightBlue)
End Sub
    ...
    If dataType = 0 Then ' own inventory
        For i = 1 To MAX_INV
            ...
            If TempPlayer(index).TradeOffer(i).Num > 0 Then
                ' currency?
                If Item(TempPlayer(index).TradeOffer(i).Num).Type = ItemCurrency Then
    ...
    ElseIf dataType = 1 Then ' other inventory
        For i = 1 To MAX_INV
            ...
            If GetPlayerInvItemNum(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Num) > 0 Then
                ' currency?
                If Item(GetPlayerInvItemNum(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Num)).Type = ItemCurrency Then
