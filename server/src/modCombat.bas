...
Public Function CanPlayerAttackNpc(ByVal attacker As Long, ByVal mapNpcNum As Long, Optional ByVal IsSpell As Boolean = False) As Boolean
    ...
    If MapNpc(mapnum).Npc(mapNpcNum).Vital(Vitals.HP) <= 0 Then
        If Npc(MapNpc(mapnum).Npc(mapNpcNum).Num).Behaviour <> AIFriendly Then
            If Npc(MapNpc(mapnum).Npc(mapNpcNum).Num).Behaviour <> AIImmobile Then
                Exit Function
            End If
        End If
    End If
    ...
    If IsPlaying(attacker) Then
        ...
        If IsSpell Then
             If npcNum > 0 Then
                If Npc(npcNum).Behaviour <> AIFriendly And Npc(npcNum).Behaviour <> AIImmobile Then
...
Public Sub PlayerAttackNpc(ByVal attacker As Long, ByVal mapNpcNum As Long, ByVal Damage As Long, Optional ByVal spellnum As Long, Optional ByVal overTime As Boolean = False)
        ...
        For i = 1 To Player_HighIndex
            If IsPlaying(i) And IsConnected(i) Then
                If Player(i).Map = mapnum Then
                    If TempPlayer(i).targetType = TargetNPC Then
                        If TempPlayer(i).target = mapNpcNum Then
                            TempPlayer(i).target = 0
                            TempPlayer(i).targetType = TargetNone
                            SendTarget i
                        End If
                    End If
                End If
    ...
    Else
        ...
        If n > 0 Then
            If Not overTime Then
                If spellnum = 0 Then Call SendAnimation(mapnum, Item(GetPlayerEquipment(attacker, Weapon)).Animation, 0, 0, TargetNPC, mapNpcNum)
            End If
        End If
        ...
        If Npc(MapNpc(mapnum).Npc(mapNpcNum).Num).Behaviour = AIAssist Then
...
Sub NpcAttackPlayer(ByVal mapNpcNum As Long, ByVal victim As Long, ByVal Damage As Long)
    ...
    Else
        ...
        Call SendAnimation(mapnum, Npc(MapNpc(GetPlayerMap(victim)).Npc(mapNpcNum).Num).Animation, 0, 0, targetPlayer, victim)
...
Function CanPlayerAttackPlayer(ByVal attacker As Long, ByVal victim As Long, Optional ByVal IsSpell As Boolean = False) As Boolean
    ...
    If Not Map(GetPlayerMap(attacker)).Moral = MoralNone Then
...
Sub PlayerAttackPlayer(ByVal attacker As Long, ByVal victim As Long, ByVal Damage As Long, Optional ByVal spellnum As Long = 0)
    ...
    If Damage >= GetPlayerVital(victim, Vitals.HP) Then
        ...
        ' send animation
        If n > 0 Then
            If spellnum = 0 Then Call SendAnimation(GetPlayerMap(victim), Item(GetPlayerEquipment(attacker, Weapon)).Animation, 0, 0, targetPlayer, victim)
        End If
        ...
        For i = 1 To Player_HighIndex
            If IsPlaying(i) And IsConnected(i) Then
                If Player(i).Map = GetPlayerMap(attacker) Then
                    If TempPlayer(i).target = targetPlayer Then
...
Public Sub BufferSpell(ByVal index As Long, ByVal spellslot As Long)
        ...
        Case 2, 3 ' targeted & targeted AOE
            ...
            If targetType = targetPlayer Then
                ...
                Else
                    ...
                    If Spell(spellnum).Type <> SpellDamageHP And Spell(spellnum).Type <> SpellDamageMP Then
                        HasBuffered = True
                    Else
    ...
    If HasBuffered Then
        SendAnimation mapnum, Spell(spellnum).CastAnim, 0, 0, targetPlayer, index
        SendActionMsg mapnum, "Casting " & Trim$(Spell(spellnum).Name) & "!", BrightRed, MsgScroll, GetPlayerX(index) * 32, GetPlayerY(index) * 32
...
Public Sub CastSpell(ByVal index As Long, ByVal spellslot As Long, ByVal target As Long, ByVal targetType As Byte)
        ...
        Case 0 ' self-cast target
            Select Case Spell(spellnum).Type
                ...
                Case SpellHealMP
                    ...
                    DidCast = True
                Case SpellWarp
                    SendAnimation mapnum, Spell(spellnum).SpellAnim, 0, 0, targetPlayer, index
                    PlayerWarp index, Spell(spellnum).Map, Spell(spellnum).x, Spell(spellnum).y
                    SendAnimation GetPlayerMap(index), Spell(spellnum).SpellAnim, 0, 0, targetPlayer, index
                    DidCast = True
            End Select
            ...
            Select Case Spell(spellnum).Type
                Case SpellDamageHP
                                ...
                                If GetPlayerMap(i) = GetPlayerMap(index) Then
                                    If isInRange(AoE, x, y, GetPlayerX(i), GetPlayerY(i)) Then
                                        If CanPlayerAttackPlayer(index, i, True) Then
                                            SendAnimation mapnum, Spell(spellnum).SpellAnim, 0, 0, targetPlayer, i
                    ...
                    For i = 1 To MAX_MAP_NPCS
                            ...
                            If MapNpc(mapnum).Npc(i).Vital(HP) > 0 Then
                                If isInRange(AoE, x, y, MapNpc(mapnum).Npc(i).x, MapNpc(mapnum).Npc(i).y) Then
                                    If CanPlayerAttackNpc(index, i, True) Then
                                        SendAnimation mapnum, Spell(spellnum).SpellAnim, 0, 0, TargetNPC, i
                        ...
                        End If
                    Next
                Case SpellHealHP, SpellHealMP, SpellDamageMP
                    If Spell(spellnum).Type = SpellHealHP Then
                        VitalType = Vitals.HP
                        increment = True
                    ElseIf Spell(spellnum).Type = SpellHealMP Then
                        VitalType = Vitals.MP
                        increment = True
                    ElseIf Spell(spellnum).Type = SpellDamageMP Then
                        VitalType = Vitals.MP
                        increment = False
                    End If
            ...
            Select Case Spell(spellnum).Type
                Case SpellDamageHP
                    If targetType = targetPlayer Then
                        If CanPlayerAttackPlayer(index, target, True) Then
                            If Vital > 0 Then
                                SendAnimation mapnum, Spell(spellnum).SpellAnim, 0, 0, targetPlayer, target
                    ...
                    Else
                        If CanPlayerAttackNpc(index, target, True) Then
                            If Vital > 0 Then
                                SendAnimation mapnum, Spell(spellnum).SpellAnim, 0, 0, TargetNPC, target
                ...
                Case SpellDamageMP, SpellHealMP, SpellHealHP
                    If Spell(spellnum).Type = SpellDamageMP Then
                        VitalType = Vitals.MP
                        increment = False
                        DidCast = True ' <--- Fixed!
                    ElseIf Spell(spellnum).Type = SpellHealMP Then
                        ...
                        increment = True
                        DidCast = True ' <--- Fixed!
                    ElseIf Spell(spellnum).Type = SpellHealHP Then
                        VitalType = Vitals.HP
                        increment = True
                        DidCast = True ' <--- Fixed!
                    End If
    ...
    If DidCast Then
        ...
        SendActionMsg mapnum, Trim$(Spell(spellnum).Name) & "!", BrightRed, MsgScroll, GetPlayerX(index) * 32, GetPlayerY(index) * 32
    End If
End Sub
...
Dim Colour As Long
    ...
    If Damage > 0 Then
        ...
        SendAnimation GetPlayerMap(index), Spell(spellnum).SpellAnim, 0, 0, targetPlayer, index
        SendActionMsg GetPlayerMap(index), sSymbol & Damage, Colour, MsgScroll, GetPlayerX(index) * 32, GetPlayerY(index) * 32
...
Dim Colour As Long
    ...
    If Damage > 0 Then
        ...
        SendAnimation mapnum, Spell(spellnum).SpellAnim, 0, 0, TargetNPC, index
        SendActionMsg mapnum, sSymbol & Damage, Colour, MsgScroll, MapNpc(mapnum).Npc(index).x * 32, MapNpc(mapnum).Npc(index).y * 32
...
Public Sub HandleHoT_Player(ByVal index As Long, ByVal hotNum As Long)
    With TempPlayer(index).HoT(hotNum)
        If .Used And .Spell > 0 Then
            ' time to tick?
            If GetTickCount > .Timer + (Spell(.Spell).Interval * 1000) Then
                If Spell(.Spell).Type = SpellHealHP Then
                   SendActionMsg Player(index).Map, "+" & Spell(.Spell).Vital, BrightGreen, MsgScroll, Player(index).x * 32, Player(index).y * 32
                   SetPlayerVital index, Vitals.HP, GetPlayerVital(index, Vitals.HP) + Spell(.Spell).Vital
                Else
                   SendActionMsg Player(index).Map, "+" & Spell(.Spell).Vital, BrightBlue, MsgScroll, Player(index).x * 32, Player(index).y * 32
...
Public Sub HandleHoT_Npc(ByVal mapnum As Long, ByVal index As Long, ByVal hotNum As Long)
    With MapNpc(mapnum).Npc(index).HoT(hotNum)
        If .Used And .Spell > 0 Then
            ...
            If GetTickCount > .Timer + (Spell(.Spell).Interval * 1000) Then
                If Spell(.Spell).Type = SpellHealHP Then
                    SendActionMsg mapnum, "+" & Spell(.Spell).Vital, BrightGreen, MsgScroll, MapNpc(mapnum).Npc(index).x * 32, MapNpc(mapnum).Npc(index).y * 32
                    MapNpc(mapnum).Npc(index).Vital(Vitals.HP) = MapNpc(mapnum).Npc(index).Vital(Vitals.HP) + Spell(.Spell).Vital
                Else
                    SendActionMsg mapnum, "+" & Spell(.Spell).Vital, BrightBlue, MsgScroll, MapNpc(mapnum).Npc(index).x * 32, MapNpc(mapnum).Npc(index).y * 32
